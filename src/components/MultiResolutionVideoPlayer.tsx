...
interface MultiResolutionVideoPlayerProps {
  src: string;
...
const MultiResolutionVideoPlayer = ({ 
  src, 
  resolutions, 
  poster, 
  className, 
  title 
}: MultiResolutionVideoPlayerProps) => {
  ...
  const setupQualityLevels = useCallback(() => {
    if (resolutions) {
      const qualities: QualityLevel[] = Object.entries(resolutions).map(([label, data]) => ({
        label,
        height: parseInt(label.replace('p', '')),
        url: data.url,
        bitrate: data.bitrate
      })).sort((a, b) => b.height - a.height); // Sort by quality descending

      setAvailableQualities(qualities);
      
      // Set default quality to 720p if available, otherwise highest
      const defaultQuality = qualities.find(q => q.label === '720p') || qualities[0];
      if (defaultQuality) {
        setCurrentQuality(defaultQuality.label);
  ...
  const initializeHLS = useCallback((sourceUrl: string) => {
    ...
    if (Hls.isSupported()) {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, (event, data) => {
        ...
        // Set up quality levels from HLS manifest
        const hlsQualities: QualityLevel[] = data.levels.map((level: any, index: number) => ({
        ...
        setAvailableQualities([
          { label: 'Auto', height: 0, url: sourceUrl, bitrate: 0 },
          ...hlsQualities
        ]);
      ...
      hls.on(Hls.Events.LEVEL_SWITCHED, (event, data) => {
        ...
        if (level >= 0 && data.level < hls.levels.length) {
  ...
  useEffect(() => {
    ...
    const initializeVideo = async () => {
      try {
        ...
        // Setup quality levels first
        if (resolutions) {
          finalSrc = setupQualityLevels();
        }
        
        if (src.includes('wasabisys.com')) {
          console.log('Generating presigned URL for Wasabi video...');
          finalSrc = await generatePresignedVideoUrl(finalSrc);
    ...
    initializeVideo();
  }, [src, resolutions, setupQualityLevels, initializeHLS]);
  ...
  const changeQuality = useCallback(async (qualityLabel: string) => {
    const video = videoRef.current;
    if (!video) return;

    if (isAdaptive && hlsRef.current) {
      // HLS quality change
      if (qualityLabel === 'Auto') {
        hlsRef.current.currentLevel = -1;
      } else {
        const levelIndex = hlsRef.current.levels.findIndex(level => 
          level.height === parseInt(qualityLabel.replace('p', ''))
        );
        if (levelIndex >= 0) {
      ...
      // Manual quality change for multi-resolution videos
      const quality = availableQualities.find(q => q.label === qualityLabel);
      if (quality && quality.url !== videoSrc) {
        ...
        try {
          let finalSrc = quality.url;
          if (quality.url.includes('wasabisys.com')) {
            finalSrc = await generatePresignedVideoUrl(quality.url);
          }
    ...
    setShowSettings(false);
  }, [isAdaptive, availableQualities, videoSrc]);
                    ...
                    {availableQualities.map((quality) => (
                      <div
                        ...
                        className={`px-3 py-2 text-sm cursor-pointer hover:bg-white/10 rounded transition-colors ${
                          currentQuality === quality.label ? 'bg-red-500/20 text-red-400' : 'text-white'
                        }`}
                      ...
                      >
                        {quality.label === 'Auto' ? (
                          ...
                          `${quality.label} (${quality.bitrate}k)`
                        )}
                      </div>
                    ))}
                  </div>
...
export default MultiResolutionVideoPlayer;